...
async def health_check():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Helper function to check if a required model matches any loaded model
            # Handles both exact matches and model:tag format
            def model_available(required_model: str, available_models: List[str]) -> bool:
                for available in available_models:
                    # Exact match
                    if required_model == available:
                        return True
                    # Match without tag (e.g., "llama3" matches "llama3:8b")
                    if available.startswith(f"{required_model}:"):
                        return True
                    # Match base name (e.g., "nomic-embed-text" matches "nomic-embed-text:latest")
                    if available.split(":")[0] == required_model:
                        return True
                return False

            required_models = [settings.pm_model, settings.embedding_model]
            models_ok = all(model_available(req, model_names) for req in required_models)
